...
- 配布性ルール **intra-bundle 例外の厳格化**: 上記 scope clarification の「同一 bundle 内 sibling skill 名参照は配布性違反ではない」例外は、参照先が **実際に `dev-workflow-bundle` の `skills` 配列メンバー**（現在: `ask-peer` / `dev-workflow` / `extract-rules` / `rules-review` / `tidy`）である場合に**限る**。`.claude/skills/<name>/` 配下の **project-local skill**（marketplace.json 未登録・bundle `skills` 配列に不在 — 例: `publicity-review` / `verify-diff` / `skill-review`）を配布 skill prose（plugin source `skills/<name>/**` 配下の SKILL.md / `references/*.md`）から参照すると、配布された skill を使う他プロジェクトに当該 skill が存在せず **dangling reference**（distribution leak）になる。判断軸: 「intra-bundle sibling だから OK」と reject する前に、参照先が bundle の `skills` 配列に列挙されているかを `jq -r '(.plugins[] | select(.name == "dev-workflow-bundle") | .skills[])' .claude-plugin/marketplace.json` で確認する — 列挙されていなければ例外は適用されず違反として扱い、参照を削除して rationale を self-contained に書き換える。rules-review / code-review の両 pass がこの例外を「sibling skill だから」と誤適用して通過させた前例があるため、membership 確認を明示の gate にする
