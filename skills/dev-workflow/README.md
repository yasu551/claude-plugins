...
A skill that guides `plan → review → implement → check/test → code review → rules update` in a single command. Behavior is customizable per project via `dev-workflow.local.md`.
...
| Settings save | Writes to `.claude/dev-workflow.local.md` |
...
| Scope | Path | Priority |
| --- | --- | --- |
| Project | `.claude/dev-workflow.local.md` | High |
| User | `~/.claude/dev-workflow.local.md` | Low |

If both exist, **project-level** settings take precedence. Adding `.claude/*.local.md` to `.gitignore` is recommended.
...
- Adding `.claude/*.local.md` to `.gitignore` is recommended
