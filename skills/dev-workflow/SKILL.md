...
- **language**: Optional. Output language code (e.g. `ja`, `en`) for Step 9.5 user-facing prose. Resolution: merged skill config → Claude Code settings (`~/.claude/settings.json` → `language` field) → default `ja`. `null` / empty string / non-string values fall through to the next resolution step. Scope and the English-token stability contract are defined in `references/self-retrospective.md` §1 item 5 / §2.1 step 5 / §5. Has no observable effect unless `self_retrospective.feedback` is also configured AND the task is not assessed as Simple difficulty
